...
```
The first half is just packing the values into three `__mm256` vector registers. Another thing to point out is that the `_MM_SHUFFLE(args)` takes the arguments and and packs them into an 8 bit immediate value. Thus first call to `_MM_SHUFFLE(2,1,3,2);` yields $2<<6 | 1<<4 | 3<<2 | 2<<0 \rightarrow  0\text{b}b10011110$.
