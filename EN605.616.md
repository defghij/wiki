...
1. _Does this protocol satisfy mutual exclusion?_
  Yes.
  
2. _Is this protocol starvation-free?_ No.
...
This is possible if thread A reads _busy_ then is suspended by the operating system or otherwise pre-empted since _busy is not an atomic register. This is a race condition within the lock algorithm as implemented above. Now we turn to the temporal ordering of accesses to _turn_.
