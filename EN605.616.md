...
Recall that a partial order $\rightarrow$ is irreflexive and transitive. That is some event $x$ cannot precede itself (irreflexive). A _total order_ $<$ on $\mathbb{X}$ is a partial order such that $\forall x,y\in\mathbb{X}, x != y$ and $(x < y)$ or $(y < x)$.
