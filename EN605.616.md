...
```
Once $flag[B]$ is set to $true$ is remains $true$. It follow that the last relation holds since otherwise thread $A$ could not have read $flag[B]$ as $false$. The following follows from the transitivity of the precedence operator $\nprec$
