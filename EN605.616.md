...
Frist, recognize that for a parallizable portion $\beta$ and serial portion $\alpha$ we have $1 - \alpha = \beta$. Second, for any optimization factor $\Delta$ on $\alpha$ we have the optimized runtime $\alpha'=\frac{\alpha}{\Delta}$. Notice that when the optimization factor $\Delta$ is $1$ we have a tautology.  

$$
\begin{align*}
s'_{n} > \Deltas_{n} &\iff \frac{1}{\frac{alpha}{k} + \frac{1 - \frac{\alpha}{k}}{n}} > \Delta(\frac{1}{\alpha + \frac{1 - \alpha}{n}})\\
...
$$
\begin{align*}
k &> \frac{2 \times 0.3}{(0.3 + \frac{1 - 2}{n-1})} \\
     \frac{0.6}{0.3 - \frac{1}{n-1}}
\end{align*}
$$

Naturally, this is a function of $n$ as we have the parallel portion of the code to consider. But, in this problem we were asked to consider perfect parallelization. So that leads our final result: $k &> \lim_{n\to\infty} \frac{0.6}{0.3 - \frac{1}{n-1}} = 2$. Thus, we want to make the serial portion of the program $k>2$ times faster.
...
$$
\begin{align*}
\Delta s_{n} = \frac{kn}{n\alpha + k(1 - \alpha)} &\iff \Delta s'_{n}(n\alpha + k(1 - \alpha)) = \frac{kn} \\
