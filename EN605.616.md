...
_Proof_: For a contradiction, suppose, without loss of generality, that $A$ funs forever in the `lock()` method. It must be executing the `while` statement, waiting until either `flag[B]` becomes `false` or `victim` is set to `B`.
What is `B` doing while `A` failed to make progress? Perhaps `B` is repeatedly entering and leaving its critical section. Is so, then `B` sets `victim` to `B` as soon as it reenters the critical section. Once `victim` is set to `B`, it does not change and `A` much eventually return from the `lock()` method-- a contradiction.

So it must be that `B` is also stuck in its `lock()` method call, waiting until either `flag[A]` becomes `false` or `victim` is set to `A`. But `victim` cannot be both `A` and `B`-- a contradiction. $_{\blacksquare}$
