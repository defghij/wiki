...
If we assume that $s_{n} = 1 \implies \Delta [s_](s_){n} = 2s_{n} = 2$ for the program taking _half_ as long. Recall that we sped the serial portion up by $k = 3$ fold. Thus:
