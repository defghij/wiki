...
_Proof_: By way of contradition, assume it does not have the mutual exclusion property. Then $\exists j,k\in\mathbb{N}: \neg(CS^{j}_{A} \rightarrow CS^{k}_{B}) & \neg(CS^{k}_{B} \rightarrow CS^{j}_{A})$. Consider each thread's last execution of _lock_ prior to entering its $k^{th},j^{th}$ critical section. We see that:
```math
A(flag[A]\xrightarrow{W}true) \rightarrow A(flag[B]\xleftarrow{R}false) \rightarrow CS_{A}
```
