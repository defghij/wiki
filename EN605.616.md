...
_Definition 3.6.1_: A history $\mathcal{H}$ is linearizable if it has an extension $\mathcal{H}'$ and there is a legal sequential history $\mathcal{S}$ such that:
...
_Theorem 3.6.1_: $\mathcal{H}$ is linearizable $\iff \forall x\in\mathcal{H}, \mathcal{H}|x$ is linearizable.

Linearizability is a _nonblocking_ property: a pending invocation of a total method is never required to wait for another pending invocation to complete.

_Theorem 3.6.2_: Let $inv(x)$ be an invocation of a total method. If $\langle x\quad inv\quad P \rangle$ is a pending invocation in a linearizable history $\mathcal{H}$, then $\exists \langle x\quad res\quad P \rangle :\quad \mathcal{H}\cdot\langle x\quad res\quad P \rangle$ is linearizable.
