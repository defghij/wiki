...
_Theorem 3.6.2_: Let $inv(x)$ be an invocation of a total method. If $\langle x inv P \rangle$ is a pending invocation in a linearizable history $\mathcal{H}$, then $\exists \langle res P \rangle: \mathcal{H}\cdot\langle x res P \rangle$ is linearizable.
