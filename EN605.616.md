...
_Definition 5.7.1_: $\mathcal{F}\in Common2 \implies \forall \mathcal{f}_{i},\mathcal{f}_{j}\in\mathcal{F}: \mathcal{f}_{i}\circ\mathcal{f}_{j} = \mathcal{f}_{j}\circ\mathcal{f}_{i} \vee \mathcal{f}_{i}\circ\mathcal{f}_{j} = \mathcal{f}_{i}$

In English, $\mathcal{F}$ is in _Common2_ if function composition commutes or $\mathcal{f}_{i}$ overwrites $\mathcal{f}_{j}$ in function composition.
