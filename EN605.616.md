...
Recall that a _cycle_ in a directed graph is a series nodes $n_{0}, \hdots, n_{k}$ such that ther are directed edges from $n_{0}$ to $n_{1}$, from $n_{1}$ to $n_{2}$, and so on up node $n_{k-1}$ connected to $n_{k}$, and finally from $n_{k}$ to $n_{0}$.
