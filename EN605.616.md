...
_Proof_: By way of contradition, assume it does not have the mutual exclusion property. Then $\exists j,k\in\mathbb{N}: CS^{j}_{A} \not\rightarrow CS^{k}_{B} \and CS^{k}_{B} \not\rightarrow CS^{j}_{A}$
