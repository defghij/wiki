...
We'll use $A(v \xrightarrow{R} x)$ to denote the event in which a thread $A$ assigns a value $v$ to some field $x$. Similiarly, we'll use $A(v \leftarrow x)$ to indicate that thread $A$ reads value $v$ from field $x$. Thought of another way, $A$ stores $v$ in field $x$ or loads $v$ from from $x$.

_Lemma_ 2.3.1: _LockOne_ algorithm satisfies mutual exclusion.
_Proof_: By way of contradition, assume it does not have the mutual exclusion property. Then $\exists j,k\in\mathbb{N}: CS^{j}_{A} \not \rightarrow CS^{k}_{B} & CS^{k}_{B} \not \rightarrow CS^{j}_{A}$
