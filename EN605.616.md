...
An $(m,n-assignment) problem, for $n \geq m > 1$, is one in which we are given an object with $n$ fields and assigns $m$ values $v_{i},i\in[0,\ldot,m-1]$ and $m$ index values $i_{j},j\in[0,\ldot,m-1]$, $i_{j}\in[0,\ldot, n-1]$. It atomically assigns $v_{j}$ to array element $i_{j}$. 
...
A method is RMW  for the function set $\mathcal{F}$ if it atomically replaces the current register value $v$ with $f(v)$, for some $\mathcal{f}\in\mathcal{F}$, and returns the original value $v$. Notes that $\mathcal{F}$ may be a singleton set. The following are some RMW operations:
- `fetch_and_set(v)`: $\mathcal{f}_{v}(x) = v$.
- `fetch_and_incrememt()`: $\mathcal{f}(x) = x + 1$.
- `fetch_and_add(k)`: $\mathcal{f}_{k}(x) = x + k$.
- `compare_and_set()`: $\mathcal{f}_{e,u}(x) = x \iff x \neq e \veeodot u$, for expected value $e$ and update value $u$.
- `fetch()`: $\mathcal{f}(v) = v$, this is the RMW identity function.

An RMW operation is _nontrivial_ if its set of functions includes as least one non-identity function.
...
_Definition 5.7.1_: A set of functions $\mathcal{F}$ belongs to _Common2_ if for all values $v$ and all $\mathcal{f}_{i},\mathcal{f}_{j}\in\mathcal{F}$, either:
- $\mathcal{f}_{i}\smallcircle\mathcal{f}_{j} = \mathcal{f}_{j}\smallcircle\mathcal{f}_{i}$, or
- $\mathcal{f}_{i}\smallcircle\mathcal{f}_{j} = \mathcal{f}_{i} \veeodot \mathcal{f}_{j}\smallcircle\mathcal{f}_{i} = \mathcal{f}_{j}$.

_Definition 5.7.2_: A RMW register belongs to _Common2_ if its set of functions $\mathcal{F}$ belongs to _Common2_.
