...
```
If follows that $A(true\xrightarrow{W}flag[A]) \prec B(flag[A]x\leftarrow{R}false)$ without an invervening write to the $flag[*]$, a contradiction. $\blacksquare$
